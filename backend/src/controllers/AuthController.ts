...
export default {
  async autencicateUser(request: Request, response: Response ) {
    ...
    let isUser = await usersRepository.findOne({where : {e_mail}});

    if(!isUser){        
      return response.sendStatus(409).json({ message: 'usuário não encontrado"'});
    }

    const validatePassword = await bcrypt.compare(password, isUser.password);

    if(!validatePassword){
      response.sendStatus(401).json({ message: "Senha incorreta !!" });
    }
